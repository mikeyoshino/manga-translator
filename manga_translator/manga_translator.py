...
class MangaTranslator:
    ...
    async def _dispatch_with_context(self, config: Config, texts: list[str], ctx: Context):
        ...
        # Prepend manga context (character/relationship info) if available
        if config.translator.manga_context:
            prev_ctx = (config.translator.manga_context + "\n\n" + prev_ctx) if prev_ctx else config.translator.manga_context
    ...
    async def _run_mask_refinement(self, config: Config, ctx: Context):
        return await dispatch_mask_refinement(ctx.text_regions, ctx.img_rgb, ctx.mask_raw, 'fit_text',
                                              config.mask_dilation_offset, config.ocr.ignore_bubble, self.verbose, self.kernel_size or config.kernel_size)
